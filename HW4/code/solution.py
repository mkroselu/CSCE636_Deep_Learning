...
class PCA():
    ...
    def _do_pca(self):
        ...
        ### YOUR CODE HERE
        # Center the data
        X_centered = self.X - np.mean(self.X, axis=1, keepdims=True)
...
class AE(nn.Module):
    ...
    def _network(self):
        ...
        self.encoder = nn.Linear(self.n_features, self.d_hidden_rep)
        self.decoder = nn.Linear(self.d_hidden_rep, self.n_features)

        # Initialize weights using Kaiming initialization
        nn.init.kaiming_normal_(self.encoder.weight)
        nn.init.kaiming_normal_(self.decoder.weight)

        # Define and initialize the weight matrix
        self.W = nn.Parameter(torch.empty(self.n_features, self.d_hidden_rep))
        nn.init.kaiming_normal_(self.W)

        self.decoder_weight = self.W.t() 
        
        
        ### Note: here for the network without weights sharing 
        
        # Define and initialize the encoder weight matrix
        self.encoder_weight = nn.Parameter(torch.empty(self.d_hidden_rep, self.n_features))
        nn.init.kaiming_normal_(self.encoder_weight)

        # Define and initialize the decoder weight matrix
        self.decoder_weight = nn.Parameter(torch.empty(self.n_features, self.d_hidden_rep))
        nn.init.kaiming_normal_(self.decoder_weight)
        

        
        ### Note: here for the network with more layers and nonlinear functions  
        
        # Define and initialize the first layer weight matrix
        self.W1 = nn.Parameter(torch.empty(self.d_hidden_rep, self.n_features))
        nn.init.kaiming_normal_(self.W1)

        # Define and initialize the second layer weight matrix
        self.W2 = nn.Parameter(torch.empty(self.n_features, self.d_hidden_rep))
    ...
    def _forward(self, X):
        ...
        hidden = torch.mm(self.W.t(), X) 

        # Forward pass through the decoder
        out = torch.mm(self.decoder_weight, hidden)
        ...
        # Forward pass through the encoder
        hidden = torch.mm(self.encoder_weight, X)
        ...
        hidden1 = nn.ReLU()(torch.mm(self.W1, X))

        # Forward pass through the second layer
        out = torch.mm(self.W2, hidden1)
    ...
    def reconstruction(self, X):
        ...
        '''
        _, n_samples = X.shape
        X_re = np.zeros_like(X)  # Initialize the reconstructed data matrix
        
        with torch.no_grad():
            for i in range(n_samples):
                ### YOUR CODE HERE
                # Extract one sample at a time
                curr_X = X[:, i:i+1]
                ...
                ### YOUR CODE HERE
                # Convert the reconstructed sample back to a numpy array
                curr_X_re = curr_X_re_tensor.numpy()

                # Update the reconstructed data matrix
                X_re[:, i] = curr_X_re.squeeze()  # Squeeze to remove the singleton dimension
